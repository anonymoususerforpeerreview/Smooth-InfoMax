...
from data import de_wwww_sounds, librispeech
...
def _dataloaders(dataset_options: DataSetConfig, specific_dir, train_sub_dir, test_sub_dir, shuffle):
    ...
    train_dataset = de_wwww_sounds.DewwwwDataset(
    ...
    test_dataset = de_wwww_sounds.DewwwwDataset(
        dataset_options=dataset_options,
        root=os.path.join(
            data_input_dir, f"corpus/{specific_dir}",
        ),
...
def _get_de_wwww_sounds_data_loaders(d_config: DataSetConfig, shuffle=True):
    ''' Retrieve dataloaders where audio signals are split into syllables '''
    print("Loading De wwww Sounds dataset...")
...
def get_dataloader(config: DataSetConfig, **kwargs):
    d = config.dataset
    if d == Dataset.DE_wwww:
        return _get_de_wwww_sounds_data_loaders(config, **kwargs)
    # elif d == Dataset.DE_wwww_RESHUFFLED:  # used for training CPC
    #     return _get_de_wwww_sounds_data_loaders(config, **kwargs)
    # elif d == Dataset.DE_wwww_RESHUFFLED_V2:  # used for training CPC Decoder
    #     return _get_de_wwww_sounds_data_loaders(config, **kwargs)
