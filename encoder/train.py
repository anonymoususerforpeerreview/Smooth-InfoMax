# Example usage:
# python -m encoder.train temp sim_audio_de_wwww_distr_true --overrides encoder_config.kld_weight=0.01 encoder_config.num_epochs=2 syllables_classifier_config.encoder_num=1 use_wandb=False train=True
# for cpc: cpc_audio_de_wwww
...
def train(opt: OptionsConfig, logs, model: FullModel, optimizer, train_loader, test_loader):
    ...
    for epoch in range(start_epoch, num_epochs + start_epoch):
        ...
        for step, (audio, _, _, _) in enumerate(train_loader):
            ...
            # TODO
            # if step % latent_val_idx == 0 and opt.encoder_config.dataset.dataset == Dataset.DE_wwww:
...
def _main(options: OptionsConfig):
    ...
    if USE_WANDB:
        ...
        if options.wandb_project_name == "":
            ...
            # f"SIM_ACML_de_wwww_{dataset}_V5_batchnorm"
