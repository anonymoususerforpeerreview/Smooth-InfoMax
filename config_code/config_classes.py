...
class Dataset(Enum):
    # de_wwww_sounds OR librispeech OR de_wwww_sounds_reshuffled
    LIBRISPEECH = 1
    LIBRISPEECH_SUBSET = 3
    DE_wwww = 4  # used to be 5, i think irrelevant for classification
    # DE_wwww_RESHUFFLED_V2 = 6
    STL10 = 7
...
class DataSetConfig:
    def __init__(self, dataset: Dataset, batch_size, labels: Optional[str] = None,
        ...
        if split_in_syllables:
            assert dataset in [Dataset.DE_wwww]
            "split_in_syllables can only be True for de_wwww_sounds dataset"

        if (split_in_syllables and dataset in [Dataset.DE_wwww]):
        ...
        self.labels = labels  # eg: syllables or vowels, only for de_wwww_sounds dataset
